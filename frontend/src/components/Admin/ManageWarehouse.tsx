...
import {
  ...
  disableWarehouse, disableWarehousesBulk
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter((warehouse) => 
        warehouse.warehouse_name.toLowerCase().includes(searchLower) ||
        warehouse.clerk_name.toLowerCase().includes(searchLower) ||
        warehouse.clerk_mob.toLowerCase().includes(searchLower) ||
    ...
    if (statusFilter !== "all") {
      filtered = filtered.filter((warehouse) => 
        statusFilter === "active" ? warehouse.is_active === 1 : warehouse.is_active === 0
      );
    }
...
const handleConfirmDisable = async () => {
  ...
  try {
    ...
    switch (response.status) {
      ...
      case "error":
      default:
        showSnackbar(response.message || "Failed to disable warehouse", "error");
        break;
    }
...
const handleGroupDelete = async () => {
  ...
  try {
    ...
    switch (response.status) {
      ...
      case "error":
      default:
        showSnackbar(response.message || "Failed to disable selected warehouses", "error");
        break;
    }
  ...
  try {
    ...
    filteredWarehouses.forEach((warehouse, index) => {
      const row = worksheet.addRow({
        ...
        updated_date: warehouse.updated_at ? new Date(warehouse.updated_at).toLocaleDateString() : '-'
      });
    ...
    worksheet.insertRow(1, ['Logistics Management System - Warehouses Report']);
