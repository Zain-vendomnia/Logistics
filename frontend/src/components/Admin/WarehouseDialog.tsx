...
const WarehouseFormModal = ({
  ...
  const formFields = [
    ...
    { name: "clerkName", label: "Clerk Name", type: "text" },
    { name: "clerkMob", label: "Clerk Mobile", type: "text" },
  ...
  const validate = () => {
    ...
    if (!editMode) {
      if (!formData.name) {
        ...
        newErrors.name = "Warehouse name contains invalid characters";
      }
    }

    // Clerk Name: required
    if (!formData.clerkName) {
      newErrors.clerkName = "Clerk name is required";
    }

    // Clerk Mobile: German format
    if (!formData.clerkMob) {
      newErrors.clerkMob = "Clerk mobile is required";
    } else if (!/^\+49\d{10,12}$/.test(formData.clerkMob)) {
      newErrors.clerkMob =
        "Enter valid German mobile number (starts with +49, 10–12 digits after)";
    }
      ...
      <DialogContent dividers>
        <Stack spacing={2} mt={1}>
          ...
          {formFields.map(({ name, label, type = "text", multiline }) => {
            ...
            return (
              <TextField
                ...
                inputProps={name === "clerkMob" ? { maxLength: 16 } : undefined}
                sx={{
                  "& .MuiInputBase-root": {
                    borderRadius: "8px",
                    transition: "all 0.3s ease",
          ...
          <FormControlLabel
            control={
              <Switch
                checked={(formData.is_active ?? true) === true}
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
