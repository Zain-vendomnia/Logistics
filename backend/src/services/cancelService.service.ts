...
import { CancelItem, CancelOrder, CancelOrderItem } from "../types/order.types";
...
/**
 * Common SQL query for fetching cancel orders with user info
 */
const CANCEL_ORDER_BASE_QUERY = `
  SELECT 
    co.id AS cancel_id,
    co.order_number,
    co.status,
    ...
    (SELECT COUNT(*) FROM cancels_order_items WHERE cancel_id = co.id) AS items_count,
...
/**
 * Map database row to CancelOrder response format
 */
const mapCancelOrderRow = (order: RowDataPacket): Partial<CancelOrder> => ({
  id: order.cancel_id,
...
// ==========================================
export const getAllCancelOrders = async (): Promise<Partial<CancelOrder>[]> => {
  return withConnection(async (connection) => {
    try {
      const [cancelOrders] = await connection.query<RowDataPacket[]>(
        `${CANCEL_ORDER_BASE_QUERY} ORDER BY co.created_at DESC`
      );
      return cancelOrders.map(mapCancelOrderRow);
    } catch (error) {
...
export const searchCancelByOrderNumber = async (
  orderNumber: string
): Promise<Partial<CancelOrder>[]> => {
  return withConnection(async (connection) => {
    try {
      const searchPattern = `%${orderNumber}%`;
      const [cancelOrders] = await connection.query<RowDataPacket[]>(
        `${CANCEL_ORDER_BASE_QUERY} 
      ...
      items.map(async (item) => {
        const [result] = await connection.query<ResultSetHeader>(
          `INSERT INTO cancels_order_items (cancel_id, article_sku, quantity, cancel_quantity, created_at, updated_at)
           VALUES (?, ?, ?, ?, NOW(), NOW())`,
          [cancelId, item.slmdl_articleordernumber, item.quantity, item.cancelQuantity]
        );
...
export const updateCancel = async (
  ...
  return withConnection(async (connection) => {
    ...
    if (data.cancel_quantity <= 0) {
      return { success: false, message: "Cancel quantity must be greater than 0" };
    }
...
export const deleteCancel = async (id: number): Promise<ServiceResult> => {
  return withTransaction(async (connection) => {
    ...
    await connection.query(`DELETE FROM cancels_order_items WHERE id = ?`, [id]);
    ...
    if (remainingItems[0].count === 0) {
      ...
      await connection.query(`DELETE FROM cancels_order WHERE id = ?`, [cancelId]);
    }
...
export const deleteAllCancels = async (): Promise<DeleteAllResult> => {
  return withTransaction(async (connection) => {
    ...
    const [result] = await connection.query<ResultSetHeader>(`DELETE FROM cancels_order`);
