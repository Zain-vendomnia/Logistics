...
export class GeocodingService {
  ...
  static async getLatLngFromAddress(
    address: string
  ...
  static async geocodeAllOrders(): Promise<
    {
      id: string;
      type: string;
  ...
  static async geocodeOrderUpdatedCustomer(
    _order_id: any,
    _street: any,
    _city: any,
    _zipcode: any
    ...
    for (let index = 0; index < orders.length; index++) {
      ...
      if (latLng) {
        try {
          ...
          if (affectedRows > 0) {
            console.log(
              `Successfully updated order with ID: ${order.order_id}`
            );
          } else {
  ...
  static async geocodeAllcustomer(): Promise<
    {
      id: string;
      type: string;
      address: { location_id: string; lat: number; lon: number };
