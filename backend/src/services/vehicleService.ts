...
const logDbError = (fn: string, err: any, meta?: Record<string, any>) => {
  console.error(
    `[${nowIso()}] [vehicleService.${fn}] DB_ERROR code=${
      err?.code ?? "UNKNOWN"
    } errno=${err?.errno ?? ""} msg=${err?.sqlMessage ?? err?.message ?? "NA"}`
  );
...
const mapMysqlError = (err: any, fallbackMessage: string): ServiceError => {
  switch (err?.code) {
    ...
    case "ER_ROW_IS_REFERENCED_2":
      return new ServiceError(
        "Cannot delete/update: record is referenced by other data.",
        409,
        "ER_ROW_IS_REFERENCED_2"
      );
    case "ER_BAD_FIELD_ERROR":
      return new ServiceError(
        "Internal configuration error: unknown column.",
        500,
        "ER_BAD_FIELD_ERROR"
      );
    default:
...
const fromError = (
  ...
  const se =
    err instanceof ServiceError ? err : mapMysqlError(err, fallbackMessage);
...
export const getVehicleById = async (
  id: number
): Promise<ServiceResponse<VehicleBasic>> => {
  const sql = `
    SELECT
      v.vehicle_id,
      v.capacity,
...
export const createVehicle = async (vehicle: {
  ...
  try {
    ...
    const [ins]: any = await conn.query(
      `INSERT INTO vehicle_details (${cols.join(
        ", "
      )}) VALUES (${placeholders})`,
      values
    );
    ...
    await conn.rollback();
    return fromError("createVehicle", err, "Failed to create vehicle.", {
...
export const updateVehicle = async (
  ...
  try {
    ...
    if (vehicle.capacity !== undefined) {
      sets.push("capacity = ?");
      vals.push(vehicle.capacity);
    }
    if (vehicle.miles_driven !== undefined) {
      sets.push("miles_driven = ?");
      vals.push(vehicle.miles_driven);
    }
    if (vehicle.next_service !== undefined) {
      sets.push("next_service = ?");
      vals.push(vehicle.next_service);
    }
    if (vehicle.warehouse_id !== undefined) {
      sets.push("warehouse_id = ?");
      vals.push(vehicle.warehouse_id);
    }
    if (vehicle.driver_id !== undefined) {
      sets.push("driver_id = ?");
      vals.push(vehicle.driver_id);
    }
    if (vehicle.insurance_number !== undefined) {
      sets.push("insurance_number = ?");
      vals.push(vehicle.insurance_number);
    }
    if (vehicle.insurance_expiry_date !== undefined) {
      sets.push("insurance_expiry_date = ?");
      vals.push(vehicle.insurance_expiry_date);
    }
    if (vehicle.is_active !== undefined) {
      sets.push("is_active = ?");
      vals.push(vehicle.is_active);
    }
...
export const disableVehicle = async (
  ...
  try {
    ...
    if (rows[0].is_active === 0) {
      ...
      return {
        status: "warning",
        message: `Vehicle ID - ${id} is already inactive`,
      };
    }
...
export const disableVehiclesBulk = async (
  ...
  if (invalidIds.length > 0) {
    return {
      status: "error",
      message: `Invalid vehicle IDs: ${invalidIds.join(", ")}`,
    };
  }
  ...
  try {
    ...
    if (rows.length === 0) {
      ...
      return {
        status: "error",
        message: "No vehicles found with provided IDs",
      };
    }

    const foundIds = rows.map((row: any) => row.vehicle_id);
    const notFoundIds = ids.filter((id) => !foundIds.includes(id));
    const alreadyInactiveIds = rows
      .filter((row: any) => row.is_active === 0)
      .map((row: any) => row.vehicle_id);
    const activeIds = rows
      .filter((row: any) => row.is_active === 1)
      .map((row: any) => row.vehicle_id);
