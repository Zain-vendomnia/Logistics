...
export const dynamicTourController = async (_req: Request, res: Response, jobList: Job[]) => {
  try {
    ...
    for (const jobChunk of jobChunks) {
      ...
      for (const tour of tourRes.data.tours || []) {
        ...
        const vias = stops
          .slice(1, -1)
          .map((stop: any) => `&via=${stop.location.lat},${stop.location.lng}!passThrough=true`)
          .join('');
        ...
        try {
          ...
          if (routingError instanceof Error) {
            console.error(`Routing failed for vehicle ${tour.vehicleId}:`, routingError.message);
          } else {
            console.error(`Routing failed for vehicle ${tour.vehicleId}:`, String(routingError));
          }
        }
      }
