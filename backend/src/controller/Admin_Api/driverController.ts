...
export const getAvailableDriversByDateAndWarehouse = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (!tourDate || !warehouseId) {
      return res.status(400).json({
        message: "tourDate and warehouseId are required query parameters.",
      });
    }
    ...
    if (isNaN(warehouseIdNum)) {
      return res
        .status(400)
        .json({ message: "Invalid warehouseId parameter." });
    }
    ...
    return res
      .status(500)
      .json({ message: "Failed to fetch available drivers." });
  }
};

export const getDriverById = async (req: Request, res: Response) => {
  try {
    const driver = await driverService.getDriverByIdAsync(
...
export const weeklyDriverPerformanceData = async (
  req: Request,
  res: Response,
) => {
  try {
